            ...
            return (
              <button key={p.id} onClick={() => onSelectProject(p.id)}
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl" style={{ background: p.color + "33" }}>
                    {p.emoji ?? p.name[0]}
                  </div>
                ...
                <div className="mt-3">
                  ...
                  <div className="h-1.5 bg-zinc-100 rounded-full overflow-hidden">
                    <div className="h-full rounded-full transition-all" style={{ width: `${prog}%`, background: progressColor(prog) }}/>
                  </div>
                </div>
...
function NewProjectButton({ onCreateProject }: { onCreateProject: (d: Omit<Project,"id"|"createdAt"|"updatedAt">) => void }) {
  ...
  const handle = () => {
    ...
    setName(""); setDesc(""); setColor(PROJECT_COLORS[0]); setEmoji("📁"); setOpen(false);
  };
  ...
  return (
    <div className="flex items-center gap-2 bg-white border border-zinc-200 rounded-xl px-3 py-2 shadow-sm flex-wrap">
      <div className="flex gap-1">
        {EMOJIS.map(e => (
          <button key={e} onClick={() => setEmoji(e)}
            className={`text-base rounded px-1 ${emoji === e ? "bg-zinc-100" : "hover:bg-zinc-50"}`}>
            {e}
          </button>
        ))}
      </div>
      <div className="flex gap-1">
        {PROJECT_COLORS.slice(0,5).map(c => (
          <button key={c} onClick={() => setColor(c)}
            className={`w-4 h-4 rounded-full ${color === c ? "ring-2 ring-offset-1" : ""}`}
            style={{ background: c }}/>
        ))}
      </div>
      ...
      <input value={desc} onChange={e => setDesc(e.target.value)}
        placeholder="説明" className="text-sm border-none outline-none w-28 bg-transparent text-zinc-400"/>
...
function EmptyState({ onCreateProject }: { onCreateProject: (d: Omit<Project,"id"|"createdAt"|"updatedAt">) => void }) {
  return (
    <div className="bg-white rounded-2xl border border-dashed border-zinc-300 flex flex-col items-center justify-center py-20 gap-4">
      ...
      <button
        onClick={() => onCreateProject({ name: "新しいプロジェクト", description: "", color: PROJECT_COLORS[0], emoji: "📁", memberIds: [] })}
